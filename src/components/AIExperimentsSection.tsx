...
const BrowserSkeleton = ({ children, onClose, projectName }: { children: React.ReactNode, onClose: () => void, projectName: string }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-2xl flex items-center justify-center p-md md:p-xl lg:p-xxl"
    >
      <motion.div
        ...
        className="w-full max-w-6xl aspect-video bg-glass-bg rounded-lg overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-glass-border flex flex-col"
      >
        <div className="h-12 bg-glass-bg border-b border-glass-border flex items-center justify-between px-6">
          ...
          <div className="flex-1 mx-8">
            <div className="h-7 bg-background rounded-md border border-glass-border/50 flex items-center px-4">
          ...
          <button 
            onClick={onClose}
            className="group p-2 hover:bg-muted rounded-full transition-all duration-300 text-muted hover:text-foreground use-std-cursor"
          >
  ...
  return (
    ...
    >
      <motion.div
        ...
        exit={{ opacity: 0, scale: 0.9, y: 20 }}
        transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
        className="relative w-full max-w-[320px] aspect-[9/19.5] bg-background rounded-lg border-[8px] border-glass-border shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col"
      >
        {/* Notch */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-28 h-6 bg-glass-bg rounded-b-lg z-20 flex items-center justify-center">
...
const ExperimentVisual = ({ videoUrl, onExpand }: { videoUrl: string, onExpand: () => void }) => {
  ...
  return (
    <div className="w-full h-full relative overflow-hidden rounded-lg bg-foreground border border-glass-border group cursor-none hide-cursor-label">
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center z-10 bg-foreground">
...
export const AIExperimentsSection = () => {
  ...
  return (
    <motion.section 
      ...
      className={`absolute inset-0 w-full min-h-screen bg-foreground z-10 px-md md:px-xl pt-xxl md:pt-xxl pb-xxl text-background ${expandedIndex !== null ? 'overflow-hidden' : 'overflow-y-auto'}`}
    >
      <div className="max-w-7xl mx-auto w-full">
        <div className="mb-12 md:mb-16">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h2 className="text-4xl md:text-7xl font-display font-black tracking-tighter uppercase leading-[0.9] flex flex-wrap gap-x-4">
              <span className="text-background">Experiments</span>
              ...
              return (
                <motion.div key={exp.id || i} ref={el => itemRefs.current[i] = el} onMouseEnter={() => setHoveredIndex(i)} className={`group border-b border-muted ${expandedIndex === i ? 'border-b-transparent' : ''}`}>
                  <div onClick={() => setExpandedIndex(i)} className="py-xl flex flex-col md:flex-row md:items-center gap-12 md:gap-16 use-std-cursor relative">
                      ...
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {exp.tools?.map((tool: string, toolIndex: number) => (
                            <span key={toolIndex} className="text-[10px] font-mono uppercase tracking-widest px-md py-1 bg-background text-foreground rounded-full">
                              {tool}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="flex items-center gap-8 md:min-w-[400px] justify-end">
                      <p className={`text-sm font-sans max-w-[280px] text-right transition-all duration-500 hidden md:block ${hoveredIndex === i ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'}`}>
                        {exp.description}
                      </p>
        ...
        <AnimatePresence>
          {expandedIndex !== null && experiments[expandedIndex] && (
            <motion.div key="experiment-details" initial={{ y: '100%' }} animate={{ y: '0%' }} exit={{ y: '100%' }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="fixed inset-0 bg-foreground z-50 md:hidden overflow-y-auto p-md">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {experiments[expandedIndex].tools?.map((tool: string, toolIndex: number) => (
                        <span key={toolIndex} className="text-[8px] font-mono uppercase tracking-widest px-sm py-xs bg-muted rounded-full opacity-60">{tool}</span>
                      ))}
                    </div>
                  </div>
                </div>
                <motion.div animate={{ rotate: 180 }}><ChevronDown size={24} className="text-muted" /></motion.div>
              </div>
              <div className="py-8 px-4 md:px-8 flex flex-col items-center">
                <div className="w-24 h-px bg-glass-border mb-lg"></div>
                <p className="text-sm font-sans mb-xl text-muted max-w-md text-center leading-relaxed">{experiments[expandedIndex].description}</p>
                <div className={cn(
