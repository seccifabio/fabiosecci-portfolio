...
import { Droplets, Dog, GraduationCap, X, Clock, Headphones, RefreshCw, MousePointerClick, Keyboard, Coffee } from 'lucide-react';
...
const BlurText = ({
  ...
  const from = direction === 'top' ? { filter: 'blur(10px)', opacity: 0, y: -20 } : { filter: 'blur(10px)', opacity: 0, y: 20 };
  const to = [
    ...
    >
      {lines.map((line, lineIdx) => (
        <div key={lineIdx} className="flex flex-wrap">
          {(animateBy === 'words' ? line.split(' ') : line.split('')).map((segment, index) => (
            <motion.span
              key={`${lineIdx}-${index}`}
              initial={from}
              ...
              transition={{
                duration: 0.6,
                times: [0, 0.5, 1],
                delay: ((lineIdx * 10) + index * delay) / 1000,
                ease: "easeOut"
              }}
              style={{
                display: 'inline-block',
...
const SplitRotateText = ({ text, fontSize = '4rem', fontWeight = 900, color = 'inherit', delay = 0 }) => {
  const words = text.split(" ");
  let globalCharCount = 0;
  
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', rowGap: '0.2em', columnGap: '0.3em' }}>
      {words.map((word, wordIdx) => (
        <div key={wordIdx} style={{ display: 'flex' }}>
          {word.split("").map((char, charIdx) => {
            const charDelay = delay + (globalCharCount++) * 0.015;
            return (
              <motion.span
                key={`${text}-${wordIdx}-${charIdx}`}
...
const IntroPage = () => {
  ...
  const slides = [
    ...
    },
    {
      id: 6,
      content: (
        <div style={{ display: 'flex', width: '100%', height: '100%', background: 'var(--novartis-secondary)' }}>
          {/* Left Side: Content */}
          <div style={{ 
            flex: '0 0 50%', 
            display: 'flex', 
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginTop: '3rem' }}>
                <p style={{ fontSize: '1.5rem', fontWeight: 500, margin: 0, color: 'white' }}>Let's grab a coffee.</p>
                <motion.div
                ...
                >
                  <Coffee size={40} strokeWidth={1.5} color="white" />
                </motion.div>
              </div>
            </motion.div>
          </div>
  ...
  const nextSlide = (e) => {
    ...
    const type = slides[currentSlide].transition || "horizontal";
    ...
    >
      ...
      <AnimatePresence mode="popLayout" custom={{ direction, type: transitionType }} initial={false}>
        ...
        >
          <div style={{ 
              ...
              <div style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                <motion.div
                  animate={{ 
                    ...
                    x: introStage === 0 ? 0 : '0', // Keep it left aligned
                    scale: introStage === 0 ? 1 : 0.85
                  }}
                  transition={{ duration: 1.2, ease: APPLE_EASE }}
                    ...
                    <>
                      ...
                      <motion.div
                        ...
                        style={{
                          ...
                          alignItems: 'flex-end',
                          position: 'absolute',
                          bottom: 0, // Positioned at the padding edge for better visibility
                          left: '-10vw',   // Counteract parent padding
                          overflow: 'hidden',
                          pointerEvents: 'none',
                          zIndex: 16
                        }}
                      >
                        <svg 
                          ...
                          viewBox="0 0 32 31" // Cropped slightly to remove internal padding
            ...
            ) : current.layout === 'education' ? (
              <div style={{ display: 'flex', width: '100%', height: '100%', alignItems: 'center' }}>
                {/* Left Side: Headline */}
                <div style={{ flex: '0 0 40%', textAlign: 'left', paddingRight: '5vw' }}>
                  <motion.div
                  ...
                  >
                    ...
                    <div style={{ fontSize: 'clamp(4rem, 8vw, 10rem)', fontWeight: 900, lineHeight: 0.8, letterSpacing: '-0.07em', textTransform: 'uppercase' }}>
                      {renderHeadline(current.headline, current.accent)}
                    </div>
                  </motion.div>
                </div>
                ...
                {/* Right Side: List */}
                <div style={{ flex: '1', paddingLeft: '8vw', display: 'flex', flexDirection: 'column', gap: '3.5rem', borderLeft: '1px solid rgba(0,0,0,0.1)' }}>
                  {current.stages.map((stage, idx) => (
                    <motion.div
                      key={idx}
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.3 + idx * 0.15, duration: 0.8, ease: APPLE_EASE }}
                    >
      ...
      <div style={{
        ...
        <img 
          ...
          style={{ 
            height: '25px', 
            opacity: (current.background === 'white' || (current.category === '[Heritage]' && introStage === 1)) ? 0.6 : 0.8,
            filter: (current.background === 'white' || current.background === '#ffffff' || (current.category === '[Heritage]' && introStage === 1)) 
              ? 'brightness(0)' 
        ...
        {showHint && (
          <motion.div
            ...
            transition={{ 
              ...
              x: { duration: 0.8, ease: APPLE_EASE }
            }}
            style={{
              position: 'fixed',
              left: '40px',
          ...
          >
            ...
            <span style={{ 
              ...
              <Highlighter delay={1.5} color="rgba(255,255,255,0.2)">
                Click or use arrows
              </Highlighter>
            </span>
          </motion.div>
